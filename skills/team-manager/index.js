...
function getValidRoles() {
    ...
    scanDir(path.resolve(__dirname, '../../prompts'));
    scanDir(path.resolve(__dirname, '../../prompts/dynamic'));
...
/**
 * Synthesize a dynamic expert identity using template-based generation.
 * NO LLM call — avoids llm.lock contention.
 */
export async function synthesize_expert(args) {
    const domain = args?.domain || 'general';
    const baseTemplate = args?.base_template || 'coder';
    const taskDescription = args?.task_description || 'Complete the assigned task.';
    const constraints = args?.constraints || 'Do not modify files outside the project scope.';

    // Sanitize the role name
    const roleName = domain.toLowerCase().replace(/[^a-z0-9-]/g, '-').replace(/-+/g, '-').substring(0, 30);
    const fullRoleName = `team-${roleName}`;
    const dynamicDir = path.resolve(__dirname, '../../prompts/dynamic');
    const filePath = path.join(dynamicDir, `${fullRoleName}.md`);
    ...
    // Template-based identity (4-point structure)
    const identity = `<!-- tool_template: ${baseTemplate} -->
<!-- depth_cap: true -->
# ${domain} Specialist

## Identity
You are a **${domain} Specialist**, dynamically generated by Niki (Team Manager) for a specific mission.

## Strict Focus
Your ONLY job is to: ${taskDescription}

## Constraints
- ${constraints}
- You must NEVER call \`synthesize_expert\` or \`handoff_to\` to create further sub-agents.
- You must NEVER modify core Prometheus files (agent.js, prom.js, llm.js).
- When your task is complete, call \`handoff_to\` with role \`team-manager\` to return control to Niki.

## Output Expectation
Provide your results in a structured format. If you wrote code, include the file path. If you analyzed something, provide a numbered findings list.
`;

    fs.writeFileSync(filePath, identity);

    console.log(`🧠 [BRAINSTORM] Synthesized expert: ${fullRoleName} (template: ${baseTemplate})`);
    logAction('EXPERT_SYNTHESIZED', `Created dynamic expert: ${fullRoleName} for domain: ${domain}`, 'team-manager');

    return {
        status: 'success',
        role_name: roleName,
        full_role_name: fullRoleName,
        file_path: filePath,
        base_template: baseTemplate,
        message: `Expert "${fullRoleName}" created. Use handoff_to(role: "${roleName}", context: "...") to activate.`
    };
}
